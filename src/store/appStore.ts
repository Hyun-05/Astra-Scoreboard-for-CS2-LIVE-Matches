    ...
    export interface AppState {
      ...
      // --- 字体 & 标题 & 动画 ---
      bpFontFamily: string;
      setBpFontFamily: (family: string) => void;
      bpFontFile: string;
      setBpFontFile: (file: string) => void;
      bpTitle: string;
      setBpTitle: (title: string) => void;
      bpAnimSpeed: number;
      setBpAnimSpeed: (speed: number) => void;
      ...
      resetBp: () => void;
      setBp: (partial: Partial<AppState['bp']>) => void;
    }
    ...
    export const useAppStore = create<AppState>()((set, get) => ({
      ...
      setFormat: (format) => {
        ...
        get().addLog(`Changed to ${format}`, 'info');
      },
      ...
      setBp: (partial) => set(s => ({ bp: { ...s.bp, ...partial } })),
      ...
      setBpFontFamily: (family) => {
        set({ bpFontFamily: family });
        (window as any).electronAPI?.updateObsState?.({ bpFontFamily: family });
      },
      bpFontFile: 'quantico-bold.ttf',
      setBpFontFile: (file) => {
        set({ bpFontFile: file });
        (window as any).electronAPI?.updateObsState?.({ bpFontFile: file });
      },
      bpTitle: 'Ban&Pick',
      setBpTitle: (title) => {
        set({ bpTitle: title });
        (window as any).electronAPI?.updateObsState?.({ bpTitle: title });
      },
      bpAnimSpeed: 500,
      setBpAnimSpeed: (speed) => {
        set({ bpAnimSpeed: speed });
        (window as any).electronAPI?.updateObsState?.({ bpAnimSpeed: speed });
      },
      ...
      addScore: (side) => {
        ...
        const needWins = { BO1: 1, BO3: 2, BO5: 3 }[state.match.format] ?? 2;
      ...
      syncBgConfig: async () => {
        ...
        set({
          bgOpacity: config?.bgOpacity ?? 0.98,
  ...
  updatePlayers: (players) => {
    ...
    const enriched = players.map(p => ({
      ...p,
      kd: p.deaths 
        ? Math.floor((p.kills / p.deaths) * 100) / 100 
        : p.kills,
    }));

    const ctPlayers = enriched
      .filter(p => p.team === 'CT')
      .sort((a, b) => (b.adr || 0) - (a.adr || 0));
    const tPlayers = enriched
      .filter(p => p.team === 'T')
      .sort((a, b) => (b.adr || 0) - (a.adr || 0));
    ...
    set(s => ({
      ...
      hotkeysEnabled: localStorage.getItem('hotkeysEnabled') === 'true',
